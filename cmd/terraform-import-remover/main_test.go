...
`
	...
	if stats.ImportBlocksRemoved != 1 {
		t.Errorf("Expected ImportBlocksRemoved to be 1, but got %d", stats.ImportBlocksRemoved)
	}
}
...
func TestConsecutiveImportBlocks(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "terraform-consecutive-import-test")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tempDir)

	testFile := filepath.Join(tempDir, "consecutive_import.tf")
	content := `
resource "aws_instance" "web" {
  ami           = "ami-123456"
  instance_type = "t2.micro"
...
`
	...
	if strings.Contains(string(modifiedContent), "import {") {
		t.Errorf("File still contains import blocks after processing")
	}
...
func TestTrailingEmptyLines(t *testing.T) {
	...
	testFile := filepath.Join(tempDir, "trailing_import.tf")
	content := `
module "hoge" {
  source = "fuga"
}

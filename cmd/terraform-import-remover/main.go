...
func processFile(filePath string, stats *Stats) error {
	...
	// Collect byte ranges of import blocks (SrcRange excludes leading comments)
	...
	for _, block := range syntaxBody.Blocks {
		if block.Type == "import" {
			r := block.Range()
			importRanges = append(importRanges, byteRange{start: r.Start.Byte, end: r.End.Byte})
		}
	}

	importBlocksCount := len(importRanges)
	fileModified := importBlocksCount > 0
	...
	if !stats.DryRun {
		...
		if fileModified {
			// Remove import blocks from content in reverse order to preserve byte offsets
			result := make([]byte, len(content))
			copy(result, content)

			for i := len(importRanges) - 1; i >= 0; i-- {
				r := importRanges[i]
				start := r.start
				end := r.end

				// Consume leading whitespace on the same line as `import`
		...
		if fileModified || !bytes.Equal(formattedContent, content) {
			...
			if fileModified {
				stats.ImportBlocksRemoved += importBlocksCount
			}
...
func printUsage() {
	fmt.Println("Terraform Import Block Remover")
	fmt.Println("--------------------------------")
	fmt.Println("This tool recursively scans Terraform files, removes all 'import' blocks,")
	fmt.Println("and applies standard Terraform formatting to the files.")
	fmt.Println()
	fmt.Println("Usage: terraform-import-remover [options] [directory]")
...
func main() {
	...
	normalizeFlag := flag.Bool("normalize-whitespace", false, "Normalize whitespace after removing import blocks")
	...
	if *versionFlag {
		fmt.Printf("Terraform Import Block Remover v%s\n", Version)
		os.Exit(0)
	}
